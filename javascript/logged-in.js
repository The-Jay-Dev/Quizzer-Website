...
const refreshQuizzes = function () {
  ...
  if (Object.keys(currentUsernameSettings.quizzes).length > 0) {
    ...
    Object.values(currentUsernameSettings.quizzes).forEach((quiz, i) => {
      ...
      <p  id="delete-${i}" class="quiz-tab-options-container quiz-tab-options-delete scale-up ">X
       </p></div>`
      );
    ...
    addQuestionBtn.addEventListener("click", function (e) {
      if (checkIfNewQBoxesEmptyStandard() === false) {
        if (
          ...
          newMessage(
            true,
            "Success! Scroll up to view added questions",
            "timed"
          );
        } else {
          newMessage(
            false,
            "Question limit reached. Please remove some questions and try again",
            "timed"
          );
        }
      } else {
    ...
    addQuestionBtn.addEventListener("click", function (e) {
      if (checkIfNewQBoxesEmptyMC() === false) {
        if (
          ...
          newMessage(
            false,
            "Question limit reached. Please delete some questions and try again",
            "timed"
          );
        }
      } else {
