...
const next = function () {
  window.removeEventListener("keydown", nPressedCheck);
...
const checkMC = function () {
  window.addEventListener("keydown", function nPressedCheck(event) {
    if (event.key === "n") {
      console.log("l");
      next();
    }
...
const comboReset = function () {
  combo = 0;
  window.addEventListener("keydown", nPressedCheck);
};
